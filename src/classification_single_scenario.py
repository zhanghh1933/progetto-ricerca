...
f = open('D:\\Top Secret\\Università\\progetto_ricerca\\script\\report3\\report_single.tex', 'w')
...
X_train, X_test, y_train, y_test = train_test_split(preprocessing.scale(dataset_shuflled[:, 1:]), np.array(dataset_shuflled[:, :1].ravel()).astype(int), test_size=0.3, random_state=0)
scoring = 'accuracy'
...
plt.savefig('D:\\Top Secret\\Università\\progetto_ricerca\\script\\report\\lr_initial.png')
plt.close()
...
plt.savefig('D:\\Top Secret\\Università\\progetto_ricerca\\script\\report\\lsvm_initial.png')
plt.close()
...
plt.savefig('D:\\Top Secret\\Università\\progetto_ricerca\\script\\report\\rf_initial.png')
plt.close()
